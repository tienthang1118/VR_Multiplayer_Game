...
public static partial class JSONTemplates {
	...
	*//*
	...
	public static Keyframe ToKeyframe(JSONObject obj){
		...
		{
			int tangentModeValue = (int)obj.GetField("tangentMode").n;
			AnimationUtility.TangentMode tangentMode = (AnimationUtility.TangentMode)tangentModeValue;
			
			AnimationCurve curve = new AnimationCurve(k);
			
			AnimationUtility.SetKeyLeftTangentMode(curve, 0, tangentMode);
			AnimationUtility.SetKeyRightTangentMode(curve, 0, tangentMode);
		}
	...
	public static JSONObject FromKeyframe(Keyframe k){
		...
		AnimationCurve curve = new AnimationCurve(new Keyframe[] { k });
		if (AnimationUtility.GetKeyLeftTangentMode(curve, 0) != 0 || AnimationUtility.GetKeyRightTangentMode(curve, 0) != 0)
		{
			int tangentMode = (int)AnimationUtility.GetKeyLeftTangentMode(curve, 0);
			result.AddField("tangentMode", tangentMode);
		}
